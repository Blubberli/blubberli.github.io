function make_slides(f) {
  ...
  slides.example1 = slide({
	...
	for (i=0; i<words.length; i++){
		...
		if (i == words.length-1){dispRow.append(".");}
		newButton.addEventListener("click", function(e) {
			...
			checkbox.value = '<font color=#008CBA>No good location</font>';
    			checkbox.id = 'loc1';
			exp.checkbox = checkbox;
			//dispRow4.append(checkbox);
			dispRow4.append("<div class = row align=center>");
			dispRow4.append(checkbox);
			dispRow4.append(" <font color=#008CBA> No good location</font></div>");
   ...
   slides.example2 = slide({
	...
	for (i=0; i<words.length; i++){
		...
		newButton.addEventListener("click", function(e) {
			...
			document.getElementById('rowp1').style.display='none';
			var g = "this is a test";
			var continuebutton = document.createElement("button");
			continuebutton.classList = 'continueButton';
			continuebutton.style = "text-align: center; align: center;";
			continuebutton.innerHTML ='Continue';
			continuebutton.addEventListener("click", function(e) {
				if (!document.getElementById('loc2').checked){
					alert("are you sure there is any good location for but not both?");
					}
				else{
				_s.button();
				}
			...
			dispRow3.append("<div align=center");
			dispRow3.append(continuebutton);
			dispRow3.append("</div>");			
			refreshButton.id = 'refresh';
			refreshButton.classList='retryButton';
			refreshButton.innerHTML='retry';
...
log_responses : function() {
 ...
 slides.startExp = slide({
     name : "startExp",
     start: function(){
     $("#instrunctionGen").html('<table id="instructionGen"> </table>');
  ...
  slides.generateEntities = slide({
	...
	for (i=0; i<words.length; i++){
		if (i<wordsBefore){
			...
			if (words[i].indexOf(",")!==-1 && words[i]!=','){w = words[i].replace(",", "");
			    if (words[i].length != 0){
				dispRow.append("<font size=5>"+w + "</font>");
				var newButton  = document.createElement("button");
    				newButton.id = i;
				newButton.classList = 'arcButton';
				newButton.innerHTML='<sub>&#8593</sub>';
					newButton.addEventListener("click", function(e) {
					w = words[this.id].replace(",", "");
					var firsthalf = words.slice(0, (parseInt(this.id))).join(" ")+" "+w;
				...
				if (words[i].length != 0){
					...
					if (i == words.length-1 && words[i] != '.') {
						if (words[i].endsWith(".")){
						var w = words[i].slice(0, words[i].length-1)}
						else { var w = words[i]}
    ...
    "response" : [exp.targetsen,exp.checkbox.checked],
